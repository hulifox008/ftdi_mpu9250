...
extern struct mpu9250_ctx mpu9250;
...
{
  uint8_t rawData[2];
  mpu9250_readbytes(&mpu9250, TEMP_OUT_H, 2, &rawData[0]);  // Read the two raw data registers sequentially into data array 
...
uint8_t ak8963_readbyte(struct mpu9250_ctx *mpu9250, uint8_t addr)
{
    mpu9250_writebyte(mpu9250, I2C_SLV0_ADDR, AK8963_ADDRESS | 0x80);
    mpu9250_writebyte(mpu9250, I2C_SLV0_REG, addr);
    mpu9250_writebyte(mpu9250, I2C_SLV0_CTRL, 0x81);
    usleep(500000);
    return mpu9250_readbyte(mpu9250, EXT_SENS_DATA_00);
}

void ak8963_writebyte(struct mpu9250_ctx *mpu9250, uint8_t addr, uint8_t value)
{
    mpu9250_writebyte(mpu9250, I2C_SLV0_ADDR, AK8963_ADDRESS); 
    mpu9250_writebyte(mpu9250, I2C_SLV0_REG, addr); 
    mpu9250_writebyte(mpu9250, I2C_SLV0_CTRL, 0x81); 
    mpu9250_writebyte(mpu9250, I2C_SLV0_DO, value); 
    mpu9250_writebyte(mpu9250, I2C_SLV0_CTRL, 0x81); 
    usleep(500000);
}
...
void setup()
{
    fprintf(stderr, "Setting up MPU9250...\n");
    ...
    fprintf(stderr, "x: %8.3f mg  %8.3f o/s\n", 1000*accelBias[0], gyroBias[0]);
...
//    fprintf(stderr, "fifo_cnt %d\n", fifo_cnt);
    ...
    ax = (float)((int16_t)(raw[i]<<8 | raw[i+1]))*aRes; // - accelBias[0];  // get actual g value, this depends on scale being set
    ...
    gx = (float)((int16_t)(raw[i+6]<<8 | raw[i+7]))*gRes;  // get actual gyro value, this depends on scale being set
  ...
  if (mpu9250_readbyte(&mpu9250, INT_STATUS) & 0x01) {  // On interrupt, check if data ready interrupt
    ...
    total_data+=(raw[0]&0x1F)<<8 | raw[1];
    mpu9250_writebyte(&mpu9250, USER_CTRL, 0x64);
...
void readAccelData(int16_t * destination)
{
  ...
  mpu9250_readbytes(&mpu9250, ACCEL_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers into data array
...
void readMagData(int16_t * destination)
{
  ...
  if(ak8963_readbyte(&mpu9250, AK8963_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
  ak8963_readbyte(&mpu9250, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
...
void initAK8963(float * destination)
{
  ...
  ak8963_writebyte(&mpu9250, AK8963_CNTL, 0x00); // Power down magnetometer  
  fprintf(stderr, "AK8963_CNTL %02X\n", ak8963_readbyte(&mpu9250, AK8963_CNTL));
  delay(10);
  ak8963_writebyte(&mpu9250, AK8963_CNTL, 0x0F); // Enter Fuse ROM access mode
  fprintf(stderr, "AK8963_CNTL %02X\n", ak8963_readbyte(&mpu9250, AK8963_CNTL));
  delay(10);
  rawData[0] = ak8963_readbyte(&mpu9250, AK8963_ASAX);  // Read the x-, y-, and z-axis calibration values
  rawData[1] = ak8963_readbyte(&mpu9250, AK8963_ASAY);
  rawData[2] = ak8963_readbyte(&mpu9250, AK8963_ASAZ); 
  fprintf(stderr, "rawData %02X %02X %02X\n", rawData[0], rawData[1], rawData[2]);
  ...
  ak8963_writebyte(&mpu9250, AK8963_CNTL, Mscale << 4 | Mmode); // Set magnetometer data resolution and sample ODR
  delay(10);
}
...
{  
#if 0
 // wake up device
  mpu9250_writebyte(&mpu9250, PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all sensors 
  delay(100); // Wait for all registers to reset 

 // get stable time source
  mpu9250_writebyte(&mpu9250, PWR_MGMT_1, 0x01);  // Auto select clock source to be PLL gyroscope reference if ready else
  delay(200); 
 ...
 // With the MPU9250, it is possible to get gyro sample rates of 32 kHz (!), 8 kHz, or 1 kHz
  mpu9250_writebyte(&mpu9250, CONFIG, 0x03);  


 // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
  mpu9250_writebyte(&mpu9250, SMPLRT_DIV, 0x04);  // Use a 200 Hz rate; a rate consistent with the filter update rate 
                                    // determined inset in CONFIG above
 
// Set gyroscope full scale range
 // Range selects FS_SEL and GFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
  uint8_t c = mpu9250_readbyte(&mpu9250, GYRO_CONFIG); // get current GYRO_CONFIG register value
 // c = c & ~0xE0; // Clear self-test bits [7:5] 
  c = c & ~0x03; // Clear Fchoice bits [1:0] 
  c = c & ~0x18; // Clear GFS bits [4:3]
  c = c | Gscale << 3; // Set full scale range for the gyro
 // c =| 0x00; // Set Fchoice for the gyro to 11 by writing its inverse to bits 1:0 of GYRO_CONFIG
  mpu9250_writebyte(&mpu9250, GYRO_CONFIG, c ); // Write new GYRO_CONFIG value to register
  
 // Set accelerometer full-scale range configuration
  c = mpu9250_readbyte(&mpu9250, ACCEL_CONFIG); // get current ACCEL_CONFIG register value
 // c = c & ~0xE0; // Clear self-test bits [7:5] 
  c = c & ~0x18;  // Clear AFS bits [4:3]
  c = c | Ascale << 3; // Set full scale range for the accelerometer 
  mpu9250_writebyte(&mpu9250, ACCEL_CONFIG, c); // Write new ACCEL_CONFIG register value

 // Set accelerometer sample rate configuration
 // It is possible to get a 4 kHz sample rate from the accelerometer by choosing 1 for
 // accel_fchoice_b bit [3]; in this case the bandwidth is 1.13 kHz
  c = mpu9250_readbyte(&mpu9250, ACCEL_CONFIG2); // get current ACCEL_CONFIG2 register value
  c = c & ~0x0F; // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])  
  c = c | 0x03;  // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz
  mpu9250_writebyte(&mpu9250, ACCEL_CONFIG2, c); // Write new ACCEL_CONFIG2 register value
 ...
 // but all these rates are further reduced by a factor of 5 to 200 Hz because of the SMPLRT_DIV setting
  ...
  // can join the I2C bus and all can be controlled by the Arduino as master
   mpu9250_writebyte(&mpu9250, INT_PIN_CFG, 0x22);    
   mpu9250_writebyte(&mpu9250, INT_ENABLE, 0x00);  // Enable data ready (bit 0) interrupt
   #endif
...
{  
    ...
    fprintf(stderr, "MPU9250 calibrating...\n");
    
    // reset device
    mpu9250_writebyte(&mpu9250, PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
    delay(100);
     
    // get stable time source; Auto select clock source to be PLL gyroscope reference if ready 
    // else use the internal oscillator, bits 2:0 = 001
    mpu9250_writebyte(&mpu9250, PWR_MGMT_1, 0x01);  
    mpu9250_writebyte(&mpu9250, PWR_MGMT_2, 0x00);
    delay(200);                                    
  
    // Configure device for bias calculation
    mpu9250_writebyte(&mpu9250, INT_ENABLE, 0x00);   // Disable all interrupts
    mpu9250_writebyte(&mpu9250, FIFO_EN, 0x00);      // Disable FIFO
    mpu9250_writebyte(&mpu9250, PWR_MGMT_1, 0x00);   // Turn on internal clock source
    mpu9250_writebyte(&mpu9250, I2C_MST_CTRL, 0x00); // Disable I2C master
    mpu9250_writebyte(&mpu9250, USER_CTRL, 0x00);    // Disable FIFO and I2C master modes
    mpu9250_writebyte(&mpu9250, USER_CTRL, 0x0C);    // Reset FIFO and DMP
    delay(15);
    
    // Configure MPU6050 gyro and accelerometer for bias calculation
    mpu9250_writebyte(&mpu9250, CONFIG, 0x03);    
    mpu9250_writebyte(&mpu9250, SMPLRT_DIV, 0x04);  // Set sample rate to 1 kHz
    mpu9250_writebyte(&mpu9250, GYRO_CONFIG, 0x00);  // Set gyro full-scale to 250 degrees per second, maximum sensitivity
    mpu9250_writebyte(&mpu9250, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity
    mpu9250_writebyte(&mpu9250, ACCEL_CONFIG2, 0x03);
...
//
  ...
  mpu9250_readbytes(&mpu9250, YA_OFFSET_H, 2, &data[0]);
  accel_bias_reg[1] = (int16_t)(data[0]<<8 | data[1]);
  mpu9250_readbytes(&mpu9250, ZA_OFFSET_H, 2, &data[0]);
  ...
  data[0] = (((uint16_t)accel_bias_reg[0] >>8)) & 0xFF;
  data[1] = (uint16_t)accel_bias_reg[0] & 0xFE;
  data[2] = (((uint16_t)accel_bias_reg[1] >>8)) & 0xFF;
  data[3] = (uint16_t)accel_bias_reg[1] & 0xFE;
  data[4] = (((uint16_t)accel_bias_reg[2] >>8)) & 0xFF;
  data[5] = (uint16_t)accel_bias_reg[2] & 0xFE;

    fprintf(stderr, "bias reg:");
  for(ii=0;ii<6;ii++) {
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
    ...
    fprintf(stderr, "MPU9250 selftest...\n");
   
    mpu9250_writebyte(&mpu9250, SMPLRT_DIV, 0x00);    // Set gyro sample rate to 1 kHz
    mpu9250_writebyte(&mpu9250, CONFIG, 0x02);        // Set gyro sample rate to 1 kHz and DLPF to 92 Hz
    mpu9250_writebyte(&mpu9250, GYRO_CONFIG, FS<<3);  // Set full scale range for the gyro to 250 dps
    mpu9250_writebyte(&mpu9250, ACCEL_CONFIG2, 0x02); // Set accelerometer rate to 1 kHz and bandwidth to 92 Hz
    mpu9250_writebyte(&mpu9250, ACCEL_CONFIG, FS<<3); // Set full scale range for the accelerometer to 2 g
  ...
  // Configure the accelerometer for self-test
        mpu9250_writebyte(&mpu9250, ACCEL_CONFIG, 0xE0); // Enable self test on all three axes and set accelerometer range to +/- 2 g
        mpu9250_writebyte(&mpu9250, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
   ...
   // Configure the gyro and accelerometer for normal operation
    mpu9250_writebyte(&mpu9250, ACCEL_CONFIG, 0x00);  
    mpu9250_writebyte(&mpu9250, GYRO_CONFIG,  0x00);  
    delay(25);  // Delay a while to let the device stabilize
 
    
    // Retrieve accelerometer and gyro factory Self-Test Code from USR_Reg
    selfTest[0] = mpu9250_readbyte(&mpu9250, SELF_TEST_X_ACCEL); // X-axis accel self-test results
    selfTest[1] = mpu9250_readbyte(&mpu9250, SELF_TEST_Y_ACCEL); // Y-axis accel self-test results
    selfTest[2] = mpu9250_readbyte(&mpu9250, SELF_TEST_Z_ACCEL); // Z-axis accel self-test results
    selfTest[3] = mpu9250_readbyte(&mpu9250, SELF_TEST_X_GYRO);  // X-axis gyro self-test results
    selfTest[4] = mpu9250_readbyte(&mpu9250, SELF_TEST_Y_GYRO);  // Y-axis gyro self-test results
    selfTest[5] = mpu9250_readbyte(&mpu9250, SELF_TEST_Z_GYRO);  // Z-axis gyro self-test results
    ...
    for(i=0;i<6;i++) {
        fprintf(stderr, "factoryTrim[i] %f\n", factoryTrim[i]);
    }
